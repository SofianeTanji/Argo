...
@testset "Language gives proper types" begin
    @variable x::R()
    @variable y::R()
    @func f(R(), R())
    @func g(R(), R())

    @test x isa Language.Variable
    @test f isa Language.FunctionType

    add_expr = f(x) + g(y)
    @test add_expr isa Language.Addition

    comp_expr = f(g(x))
    @test comp_expr isa Language.Composition

    sub_expr = f(x) - g(y)
    @test sub_expr isa Language.Addition
    @test sub_expr.terms[2] isa Language.Negation
end
